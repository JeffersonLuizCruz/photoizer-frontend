          ...
          <div key={foto.id} className="group relative">
            ...
            <div className="mt-1">
              {jaComprada ? (
                ...
                <span className="block text-center rounded py-1 text-[11px] font-medium bg-muted text-muted-foreground">
                  Aguardando confirmação
                </span>
              ) : isSelected ? (
                foto.downloadada ? (
                  <a href={ecommerceService.downloadUrl(token, foto.id)}
                    className="flex items-center justify-center gap-1 rounded text-[11px] text-emerald-600 dark:text-emerald-400 font-medium py-1 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-colors">
                    <Download className="h-3 w-3" />
                    Baixada
                  </a>
                ) : (
