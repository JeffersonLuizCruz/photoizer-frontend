          ...
          <div key={foto.id} className="group relative motion-safe:transition-all motion-safe:duration-300 motion-safe:hover:-translate-y-1.5">
            <div
              ...
              <button
                ...
                aria-label={isFavorito ? `Remover ${foto.fileName} dos favoritos` : `Adicionar ${foto.fileName} aos favoritos`}
                className="absolute top-2 right-2 p-2 rounded-full bg-black/30 hover:bg-black/60 motion-safe:transition-all motion-safe:duration-200 hover:scale-110 active:scale-95">
                <Heart
              ...
              <div className="absolute bottom-2 right-2 translate-y-0 opacity-100 sm:translate-y-2 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100 motion-safe:transition-all motion-safe:duration-300">
                <button
                  onClick={(e) => { e.stopPropagation(); onOpenComments(index) }}
                  aria-label={`Comentar em ${foto.fileName}`}
                  className="flex items-center gap-1.5 rounded-full bg-white/20 backdrop-blur px-2.5 py-2 text-xs font-medium text-white hover:bg-blue-600 motion-safe:transition-colors active:scale-95">
            ...
            <div className="mt-1.5">
              {jaComprada ? (
                ...
                <span className="block text-center rounded py-2 text-xs font-medium bg-muted text-muted-foreground">
                ...
                foto.downloadada ? (
                  ...
                  <button onClick={() => onSelect(foto.id)}
                    className="w-full rounded py-2 text-xs font-medium motion-safe:transition-colors bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-900/50">
                    Remover
                  </button>
                )
              ) : isInCart ? (
                ...
                <button onClick={() => onSelect(foto.id)}
                  className="w-full rounded py-2 text-xs font-medium motion-safe:transition-colors bg-muted text-muted-foreground hover:bg-accent">
                  Incluir
                </button>
              ) : (
                <button onClick={() => onToggleCarrinho(foto.id)} disabled={isCartLoading}
